...
// -------------------------------------------------------------------
// --- NEW REPUTATION MANAGEMENT LOGIC ---
// -------------------------------------------------------------------

// Function to increase a node's reputation score
void Node::rewardNode(const std::string& nodeId, double points) {
	// Ensure the score doesn't exceed a defined maximum (e.g., 100.0)
	// Initialize to 0.0 if not present
...
// Function to decrease a node's reputation score
void Node::penalizeNode(const std::string& nodeId, double points) {
	// Ensure the score doesn't go below a defined minimum (e.g., -100.0)
	// Initialize to 0.0 if not present
	double currentRep = getNodeReputation(nodeId);
...
// Function to retrieve a node's current reputation score
double Node::getNodeReputation(const std::string& nodeId) const {
	// Returns 0.0 if the node is not found in the map (neutral starting reputation)
...
// Helper function to simulate AI result validation
bool Node::validateInferenceResult(std::shared_ptr<Transaction> tx) const {
	// Simplified: In a real system, this would involve consensus on the AI result,
	// checking against ground truth, or comparing with other nodes' results.
	// For simulation, we'll randomly pass/fail or check a specific field.
	// Let's assume the transaction data contains a 'quality_score' field.
	// Since Transaction class is simplified, let's use a simple condition:
	// Assume a valid result if the creator's ID starts with 'a' (purely for simulation)
	return tx->getCreatorPublicKey()[0] == 'a';
}


// MODIFIED: Process Transaction to include reputation update logic
bool Node::processTransaction(std::shared_ptr<Transaction> tx) {
	...
	// (MODIFICATION) 2. Evaluate transaction and update reputation
	// This node evaluates the quality of the incoming transaction and adjusts
	// the reputation of its creator accordingly.
	// Assuming 'INFERENCE_RESULT' transactions are the ones requiring AI quality check.
	if (tx->getType() == "INFERENCE_RESULT") { // Check if the transaction type needs reputation check
		bool isResultValid = validateInferenceResult(tx); // Check the quality of the AI result
		if (isResultValid) {
			// Reward the node for a good contribution (e.g., +1.0 point)
			rewardNode(tx->getCreatorPublicKey(), 1.0); // Use the internal function
		}
		else {
			// Penalize the node for a bad/incorrect contribution (e.g., -10.0 points)
			penalizeNode(tx->getCreatorPublicKey(), 10.0); // Use the internal function
		}
	}
...
// MODIFIED: mineBlock to pass reputationScores to DAG for transaction selection
std::shared_ptr<Block> Node::mineBlock(const std::string& minterId) {
	...
	// (MODIFICATION) Pass the node's current reputation scores map to the DAG.
	// The DAG logic will use this to prioritize transactions from highly reputable nodes.
	std::vector<std::shared_ptr<Transaction>> txsToInclude = transactionDAG->getTransactionsToProcess(100, this->reputationScores);
