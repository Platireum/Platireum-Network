...
std::shared_ptr<Transaction> createDummyTransaction(const std::string& creatorPublicKey, const std::string& payload, TransactionType type = TransactionType::VALUE_TRANSFER, const std::vector<std::string>& parents = {})
{
    ...
    payload_json["inputs"] = json::array(); // Add an empty inputs array
    payload_json["outputs"] = json::array(); // Add an empty outputs array
    Transaction tx(type, creatorPublicKey, payload_json.dump(), parents, {});
...
protected:
    void SetUp() override {
        ...
        validatorManager->registerValidator(CryptoHelper::getPublicKeyHex(validatorKeys["validator1"]), 1000.0);
        validatorManager->registerValidator(CryptoHelper::getPublicKeyHex(validatorKeys["validator2"]), 2000.0);
        validatorManager->registerValidator(CryptoHelper::getPublicKeyHex(validatorKeys["validator3"]), 1500.0);

        // Initialize genesis block for finality chain
        finalityChain->initializeGenesisBlock(CryptoHelper::getPublicKeyHex(validatorKeys["validator1"]), validatorKeys["validator1"]);
    }
...
TEST_F(PoCConsensusTest, ValidatorSelectionBasedOnPower) {
    ...
    std::string val2PubKey = CryptoHelper::getPublicKeyHex(validatorKeys["validator2"]);
    ...
    int val1Picks = pickCounts[CryptoHelper::getPublicKeyHex(validatorKeys["validator1"])];
    int val3Picks = pickCounts[CryptoHelper::getPublicKeyHex(validatorKeys["validator3"])];
...
TEST_F(PoCConsensusTest, TransactionDAGIntegrationWithFinalityChain) {
    ...
    std::string val1PubKey = CryptoHelper::getPublicKeyHex(validatorKeys["validator1"]);
    ...
    for (auto const& [key, val] : confirmedTxs) {
        txs.push_back(val);
    }
    Block dummyBlock(finalityChain->getCurrentChainTipHash(), finalityChain->getCurrentHeight() + 1, "dummy_dag_root", val1PubKey, validatorKeys["validator1"], txs);
...
TEST_F(PoCConsensusTest, ProcessAIComputationProofTransaction) {
    ...
    // Node testNode("test_node", val1PubKey, validatorKeys["validator1"], validatorManager);
